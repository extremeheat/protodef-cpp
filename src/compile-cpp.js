...
`.trim()
  ...
  bitflags: {
    ...
    json (args, [name, refName], makeCallingCode) {
      ...
      for (let i = 0; i < flags.length; i++) {
        const flag = flags[i]
        if (!flag) continue
        s += `PDEF_JSON_putNumber_OR_BAIL(${flag}, ${refName}.${flag});\n`
      }
  ...
  bitfield: {
    ...
    json (args, [name, refName], makeCallingCode) {
      ...
      s += `PDEF_JSON_putStartObj_OR_BAIL(${name});\n`
      for (const v of fields) {
        s += `PDEF_JSON_putNumber_OR_BAIL(${v.name}, ${refName}.${v.name});\n`
      }
      s += 'PDEF_JSON_putEndObj_OR_BAIL\n'
      return s
    }
...
function visitRoot (root, mode, customTypes, specialVars, logging) {
  ...
  function makeToJSONStr (type, varName, isAnon) {
    ...
    if (cppType) {
      ...
      else return localVarName ? `PDEF_JSON_putNumber_OR_BAIL(${localVarName}, (int64_t)${name})` : `PDEF_JSON_putNumberAnon_OR_BAIL((int64_t)${name})`
    } else {
  ...
  function makeDecodeStr (type, varName, maybe) {
    ...
    if (protodefTypeToCppDecode[type]) {
      ...
      let s = ''
      if (maybe) s += `${name}.reset(new pdef::proto::${type}{}); pdef::proto::decode::${type}(stream, *${name})` // TODO: false here -- we don't want to realloc!
  ...
  function encodeType (fieldName, fieldType, structPaddingLevel = 0, objPath, _, objectName) {
    ...
    if (builtinEncodeFn) {
      ...
      pushToJSON(`  PDEF_JSON_putStartArr_OR_BAIL(${n}); /*J2.1*/`)
      ...
      if (typeName === 'array') {
        ...
        if (actualType[0] === 'container') {
          ...
          const vName = 'v' + (structPaddingLevel + 1)
          pushToJSON(`  PDEF_JSON_putStartArr_OR_BAIL(${n}); /*J3.1*/`)
        ...
        for (const [caseName, caseType] of Object.entries(cases)) {
          ...
          if (realName) {
            ...
            if (!Array.isArray(caseType)) {
              ...
              pushDecode(`       ${objName}.${realName}.reset(new pdef::proto::${colonJoin(objPath)}::${structureName}{}); pdef::proto::${colonJoin(objPath)}::${structureName} &${vName} = *${objName}.${realName}; /*8.4*/`)
        ...
        if (Array.isArray(fieldType)) {
          ...
          if (customTypes[actualType]) {
            ...
            pushToJSON(`  ${makeToJSONStr(Array.isArray(fieldType) ? fieldType[0] : fieldType, [objName, n], isAnon)}; /*${typeName}*/ /*4.5*/`)
