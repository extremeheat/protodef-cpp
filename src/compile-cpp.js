...
function visitRoot (root, mode, customTypes, specialVars, logging) {
  ...
  function makeDecodeStr (type, varName, maybe) {
    ...
    if (protodefTypeToCppDecode[type]) {
      ...
      let s = ''
      if (maybe) s += `${name} = {}; pdef::proto::decode::${type}(stream, *${name})` // TODO: false here -- we don't want to realloc!
  ...
  function encodeType (fieldName, fieldType, structPaddingLevel = 0, objPath, _, objectName) {
    ...
    if (builtinEncodeFn) {
      ...
      if (typeName === 'array') {
        ...
        pushEncodeAndToJSON(`  const pdef::proto::${colonJoin(objPath)}::${structureName} &v = ${isAnon ? '*' : ''}${structPropName}; /*${JSON.stringify(objPath)}*/ /*7.4*/`)
